...
def search_venues():
  ...
  num_venues = len(Venue.query.filter(Venue.name.ilike('%{}%'.format(search))).all())
...
def show_venue(venue_id):
  ...
  data = query.to_dict()
  data['upcoming_shows_count'] = len(Show.query.filter(Show.venue_id == venue_id, Show.start_time > datetime.now()).all())
  data['past_shows_count'] = len(Show.query.filter(Show.venue_id == venue_id, Show.start_time < datetime.now()).all())
